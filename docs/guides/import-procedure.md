...
기준일 2026-09-26 · MS2a·MS2b의 모든 import 티켓(NET-01, IAM-02, IAM-03, STO-01, CMP-01, CMP-02, RDB-01, CDN-02, DEP-02)에 적용

> 각 명세서에는 그 그룹에만 해당하는 항목만 적는다. 이 페이지의 절차는 모든 그룹이 똑같이 따른다.
> 시작 조건: STA-07(안전 게이트) 완료. 이전에는 어떤 그룹도 import를 시작하지 않는다.

---

## 1. 파일 규칙

그룹마다 자기 파일만 수정한다. 다른 그룹 파일을 고쳐야 하면 그 그룹 담당자에게 요청한다.
...
- 공통 파일(`envs/prod/versions.tf`, `providers.tf`, `backend.tf`, `variables.tf`, `outputs.tf`)은 STA 담당만 수정한다.
- 그룹 간 값 전달(예: network의 서브넷 ID를 database가 사용)은 상대 그룹 모듈의 output을 참조한다. 필요한 output이 없으면 해당 그룹 담당자에게 추가를 요청한다.
- import가 끝나 state에 등록된 뒤에는 `envs/prod/imports/<그룹>.tf`의 import 블록을 지워도 된다. 지우는 것은 plan "No changes" 확인 후 별도 커밋으로 한다.

## 2. apply 순서 규칙

state 파일은 하나라서 한 번에 한 사람만 apply할 수 있다.

- `import-log.md`의 그룹 상태를 `대기` → `진행 중` → `완료`로 적는다.
- apply는 `진행 중`으로 먼저 적은 사람이 한다. 동시에 `진행 중`인 그룹은 최대 1개.
- 기다리는 사람은 `terraform plan`만 실행하며 코드를 맞춘다. plan도 state 잠금을 잡으므로 동시에 실행하면 한쪽이 잠금을 못 얻어 실패할 수 있다. `terraform plan -lock-timeout=5m`처럼 잠금 대기 시간을 주고, 잠금을 끄는 옵션(`-lock=false`)은 쓰지 않는다.
- 잠금이 오래 풀리지 않으면 강제 해제(`force-unlock`)하지 말고 잠금을 잡은 사람에게 먼저 확인한다.
...
멈춘 경우 PR에 상황을 적고 STA 담당과 해당 그룹 담당이 함께 확인한다.

## 5. 그룹 PR 리뷰 체크리스트

리뷰어는 아래 항목을 모두 확인한 뒤 승인한다.

- [ ] 수정 파일이 자기 그룹 파일(1절 표)뿐이다
- [ ] plan 결과에 변경·교체·삭제가 0건이다
- [ ] 보호 대상 자원에 `prevent_destroy`가 있다
- [ ] plan 출력과 코드에 시크릿 값, 계정 ID, 개인 IP가 없다
- [ ] 명세서의 그룹 고유 기능이 모두 반영됐다
- [ ] `import-log.md`가 갱신됐다
