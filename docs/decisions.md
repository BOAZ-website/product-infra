...
| EC2-A 기존 EIP(`eipalloc-0d58d66169c7560bb`)·association을 managed로 import할지 | 결정 대기 | [확인 필요] | 서버 코드 편입 전 | compute 모듈, CloudFront api origin |
...
| `boaz-dev-frontend` | 서비스 중인 배포 없음, policy가 삭제된 배포 `E35Q1HDBWZRJF5` 참조 | 관리 대상 유지/제외/정리 | 결정 대기 |
...
- 기존 admin CloudFront 배포(`E2GM63NBDWPND0`)는 계획서 인벤토리에 포함된 import 대상이다. 계획서 비목표인 "admin 신규 환경 구성"과는 별개다.
