...
- 기존 계획에 없던 모니터링·경보를 새 자원으로 만든다.
- 기존 자원을 건드리지 않고 새로 만들기만 하므로, state·backend·CI가 제대로 동작하는지 운영 자원 import 전에 확인하는 첫 작업으로 쓴다.
- RDS 여유 메모리 부족(최저 약 25MB 실측)을 가장 먼저 감시한다.
...
**목적:** CloudWatch Agent와 SNS 경보를 Terraform으로 새로 만든다.

| 규모 | 선행 | 차단 결정 | 마일스톤 | tasks.md | GitHub 이슈 |
| --- | --- | --- | --- | --- | --- |
| 한 주 | STA-01 | 없음 | MS1 | 신규 | #13(R6) |
...
**목적:** CloudFront 접근 로그를 남기고 핵심 지표를 한 화면에서 본다. 권장 항목.

| 규모 | 선행 | 차단 결정 | 마일스톤 | tasks.md | GitHub 이슈 |
| --- | --- | --- | --- | --- | --- |
| 하루이틀 | OBS-01, CDN-02 | 없음 | MS3 | 신규 | #13(R15) |
...
- MS1 안에 state 저장소가 준비되지 않으면 콘솔로 먼저 만들고 나중에 import 대상에 추가 [추정]
