...
- 배포 번들·프론트엔드·지원서·아카이빙 S3 버킷을 코드로 관리한다.
- 버전 관리·암호화·퍼블릭 차단·수명 주기·버킷 정책을 각각 별도 리소스로 둔다.
- 관리하지 않을 버킷은 사유와 함께 제외 목록에 적는다.
...
**목적:** storage 모듈을 작성하고 서비스 버킷을 import해 plan "No changes"를 확인한다.
...
**목적:** 레거시·용도 불명 버킷을 관리 대상과 제외로 나누고 사유를 남긴다.
