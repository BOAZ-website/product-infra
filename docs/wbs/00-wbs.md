...
| `01-schedule.md` | Phase 일정. 팀원에게 Phase 단위로 일을 나눌 때 기준 |
...
- 이 표의 "완료" 항목은 티켓으로 만들지 않았음. 부분 완료 티켓(STA-01, STA-05, STA-13)은 각 명세서에 진행 내용을 적었음
...
Phase별 일정(기간·담당·목표·완료 기준)과 Phase별 티켓 순서는 `docs/wbs/01-schedule.md`(Phase 일정)에서 관리함. 전체 티켓 목록의 "Phase" 칸은 이 일정과 같음
...
| DOC-02 | 결정 레지스터 통합 | DOC | 하루이틀 | 없음 | 없음 | Phase 1 | 예 | 완료(#18) |
