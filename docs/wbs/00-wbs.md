...
> 12월 모집 시즌 전 목표는 **평시(off) 상태의 모든 자원을 import하고 `terraform plan` 결과가 "No changes"인 상태**까지. 시즌 전환 자동화와 리허설은 2027년으로 넘김
...
- `P1`~`P10`은 설계 문서(design.md)의 검증 속성 번호. 독립 마일스톤으로 두지 않고 해당 티켓의 기능으로 넣었음
...
- 12월 전에는 import 대상 명세서(NET·IAM·STO·CMP·RDB·CDN·DEP)와 `import-log.md`를 우선 정확하게 관리하고, 나머지 명세서는 해당 마일스톤에 가까워지면 보강함
...
- 1~3(MS2a)은 서로 선행 관계가 없어 3명이 동시에 시작할 수 있음. IAM만 IAM-01(권한 변경 후 재조사)을 먼저 끝냄
- 4~8(MS2b)은 SEA-01(시즌 변수 최소 구현)이 끝난 뒤 시작함. DB와 배포는 컴퓨팅과 동시에 진행할 수 있음
...
- 인원이 1명 이하로 확정되면 MS2b는 동결 뒤로 미루고, 12월 전 목표는 MS2a까지로 줄임
...
1. DOC-01 새 WBS PR #12 머지 (DOC-05 이후)
2. DOC-02 결정 레지스터 통합
