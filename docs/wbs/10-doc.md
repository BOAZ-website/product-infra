...
- GitHub 실제 상태와 문서 표기가 다른 부분을 바로잡는다.
- 흩어진 결정 목록을 하나로 합치고, 착수 전에 스펙 결함을 정정한다.
- 마지막 단계의 README·런북·계약 문서도 이 명세서에서 다룬다.
...
## DOC-01 구 WBS PR #12·이슈 #11 종료

**목적:** 구 WBS(`docs/wbs.md`, PR #12)는 `docs/wbs/`로 대체됐으므로 머지하지 않고 닫는다. 기존 "wbs.md 상태 오기 정정" 작업은 이 종료로 불필요해졌다.

| 규모 | 선행 | 차단 결정 | 마일스톤 | tasks.md | GitHub 이슈 |
| --- | --- | --- | --- | --- | --- |
| 하루이틀 | 없음 | 없음 | MS0 | 없음 | #11, PR #12 |

| 기능 ID | 기능 | 완료 확인 방법 |
| --- | --- | --- |
| DOC-01-01 | PR #12에 "`docs/wbs/`로 대체" 코멘트를 남기고 머지 없이 닫음 | `gh pr view 12`에서 CLOSED |
| DOC-01-02 | 이슈 #11을 같은 사유로 닫음 | `gh issue view 11`에서 CLOSED |

## DOC-02 결정 레지스터 통합

**목적:** decisions.md를 유일한 결정 목록으로 만들고, 빠진 항목을 추가한다.
...
**목적:** 관리자 콘솔 인프라 요구사항(#13) 중 이 작업에 포함할 것과 뺄 것을 문서에 명시한다.
...
| 기능 ID | 기능 | 완료 확인 방법 |
| --- | --- | --- |
| DOC-03-01 | 이슈 #13에 범위 경계 코멘트 등록(기존 admin 자원 import는 포함, 앱 배포 workflow 변경은 제외) | `gh issue view 13`에서 코멘트 확인 |
| DOC-03-02 | 계획서 비목표 문구에 "기존 admin 자원 import는 범위 안" 명시 (2026-09-26 `overview/migration-plan.md`에 반영됨) | 문서에서 문구 확인 |

## DOC-04 스펙 결함 일괄 정정

**목적:** 검토 보고서에서 찾은 스펙 결함 10건을 착수 전에 고친다.
...
**목적:** 이미 작업이 끝나 리뷰를 기다리는 사전 준비 PR 2건(저장소 구조, AWS 조사 기록)을 머지한다.
...
**목적:** 이 WBS를 GitHub에서 추적할 수 있게 Milestone과 이슈를 만든다.
...
| DOC-06-02 | 티켓 57개를 GitHub 이슈로 생성(제목에 티켓 키), tasks.md 대응표는 WBS 부록 A 링크로 대신 | 이슈 목록에 티켓 키 검색 결과 존재 |

## DOC-07 tasks.md 기준 문서 안내

**목적:** 작업 순서·완료 조건의 기준은 노션 명세서로 확정됐으므로, tasks.md는 내용을 고치지 않고 참고용이라는 안내만 넣는다.
...
**목적:** 처음 보는 사람이 README만 보고 plan을 실행할 수 있게 한다.
...
**목적:** 시즌 시작·종료를 사람이 순서대로 따라 할 수 있는 절차서를 만든다.
...
**목적:** 기존 배포 workflow가 참조하는 값과 Terraform output의 대응 규칙을 문서로 남긴다.
...
**목적:** 모든 결정의 최종 결과와 스펙 변경 수동 승인 절차(#13 R18)를 기록한다.
